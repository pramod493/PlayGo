...
bool QTuio::tuioToQt(TUIO::TuioCursor *tcur, QEvent::Type eventType)
{
	...
	QTouchDevice tmpTouchDevice = QTouchDevice();
	tmpTouchDevice.setType(QTouchDevice::TouchScreen);

	QList<QTouchEvent::TouchPoint> points = qTouchPointMap->values();
	QEvent *touchEvent = new QTouchEvent(eventType, &tmpTouchDevice/*QTouchDevice::TouchScreen*/,
										 Qt::NoModifier, touchPointStates, points);	//qTouchPointMap->values());
